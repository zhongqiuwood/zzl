...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    uint256 public highWaterMark = 3000e18;
    uint256 public minStake = 20e18;
    uint256 public maxLevel;
    uint256 public r1Ratio = 50;
    ...
    function earnedByWeight(address account) public view returns (uint256) {
        ...
        uint256 weight = weight[account];

        if (balance < minStake) {
            return 0;
        }

        return weight.mul(rewardPerWeight().sub(userRewardPerWeightPaid[account])).div(1e18).add(rewardsByWeight[account]);
    }
    // reward by weight
    // --------------------------------------------------------
    ...
    function getDeltaWeight(uint256 balance, uint256 amount) public view returns (uint256) {
        ...
        if (balance > min) {
            ...
            if (total >= max) {
                ...
                deltaWeight = balance + amount;
            }
            if (balance == min) {
                deltaWeight = deltaWeight - balance;
            }
        }
    ...
    function onStake(address account, uint256 balanceBeforeStake, uint256 amount) {
        if (balanceBeforeStake < minStake && (balanceBeforeStake + amount) >= minStake)  {
            totalWeight = totalWeight.add(weight[account]);
        }
    }
    
    function stake(uint256 amount, address referrer) external nonReentrant updateReward(msg.sender) {
        require(amount > 0, "Cannot stake 0");
        uint256 beforeStake = _balances[msg.sender];

        address existingReferrer = referrerMap[msg.sender];
        ...
        if (referrer != address(0)) {
            ...
            if (deltaWeight > 0) {
                for (uint24 i = 0; i < maxLevel; i++) {
                    weight[referrer] = weight[referrer].add(deltaWeight);
                    if (_balances[referrer] >= minStake) {
                        totalWeight = totalWeight.add(deltaWeight);
                    }
    ...
    function onWithdraw(address account, uint256 balanceAfterWithdraw, uint256 amount) {
        if (balanceAfterWithdraw < minStake && (balanceAfterWithdraw + amount) >= minStake)  {
            totalWeight = totalWeight.sub(weight[account]);
            require(totalWeight >= 0, "onWithdraw: invalid delta weight");
        }
    }
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        require(afterWithdraw >= 0, "Cannot withdraw more than you have");
        ...
        if (referrer != address(0)) {
            ...
            if (deltaWeight > 0) {
                for (uint24 i = 0; i < maxLevel; i++) {
                    weight[referrer] = weight[referrer].sub(deltaWeight);
                    require(weight[referrer] >= 0, "Invalid delta weight");

                    if (_balances[referrer] >= minStake) {
                        totalWeight = totalWeight.sub(deltaWeight);
                        require(totalWeight >= 0, "Invalid totalWeight and delta weight");
                    }
    ...
    function notifyRewardAmount(uint256 reward) external onlyRewardsDistribution updateReward(address(0)) {
        if (block.timestamp >= periodFinish) {
            rewardRate = reward.mul(r1Ratio).div(100).div(rewardsDuration);
            rewardRateByWeight = reward.mul(100 - r1Ratio).div(100).div(rewardsDuration);
        } else {
            uint256 remaining = periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(rewardRate);
            rewardRate = reward.mul(r1Ratio).div(100).add(leftover).div(rewardsDuration);
            uint256 leftoverw = remaining.mul(rewardRateByWeight);
            rewardRateByWeight = reward.mul(100 - r1Ratio).div(100).add(leftoverw).div(rewardsDuration);
        }
